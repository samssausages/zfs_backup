...
source_pool="workpool"  #this is the zpool in which your source dataset resides (note the does NOT start with /mnt/)
source_dataset="nginx"   #this is the name of the dataset you want to snapshot and/or replicate
...
remote_server="10.5.10.20" #remote servers name or ip
#
####################
#
###### Dump mariadb database, optional
...
mariadb_backup_location="/data/db_backup" # must always be named /data/db_backup inside of the container. Map this to a /db_backup directory
mariadb_user="root" # mariadb root user
MARIADB_ROOT_PASSWORD=$(grep -oP "^MARIADB_ROOT_PASSWORD='\K[^']+" /path/to/your/password/file/.env) #setup to pull password from .env file
#
...
destination_pool="disk22"  #this is the zpool in which your destination dataset will be created
parent_destination_dataset="backup_zfs" #this is the parent dataset in which a child dataset will be created containing the replicated data (zfs replication)
...
syncoid_receive_options="" # set recordsize & compression at destination to the max with --recvoptions=o recordsize=1M o compression=zstd-19 Must consider this when restoring, as may want to change that back
#
##########
#
...
dump_database() {
  ...
  if docker exec "$mariadb_container_name" bash -c "rm -rf $mariadb_backup_location/*"; then
    echo "All contents of $mariadb_backup_location have been deleted in container $mariadb_container_name."
  else
    echo "Error: Failed to delete contents of $mariadb_backup_location in container $mariadb_container_name."
